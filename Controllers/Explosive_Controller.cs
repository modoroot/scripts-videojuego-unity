...
public class Explosive_Controller : MonoBehaviour {
    ...
    private float growthSpeed = 15;
    private float maxSize = 6;
    private float explosionRadius;
    private bool canGrow = true;
    private void Update() {
        if (canGrow)
            transform.localScale = Vector2.Lerp(transform.localScale, new Vector2(maxSize, maxSize), growthSpeed * Time.deltaTime);
